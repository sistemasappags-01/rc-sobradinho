...
function renderSparkline(id, dias, fn, porDia, cor) {
  ...
  // Atualizar chart existente sem recriar
  ...
  // e o container tem largura correta antes de criar o canvas
  requestAnimationFrame(() => {
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      data: { labels: dias, datasets: [{ data: dados,
        borderColor: cor, borderWidth: 1.5, fill: true,
      ...
      options: {
        ...
        scales: { x:{display:false}, y:(()=>{
            // D4 — Escala mínima contextual: evita linha no centro quando todos iguais
            const vals = dados.filter(v => v !== null && v !== undefined);
            const min = Math.min(...vals), max = Math.max(...vals);
            const flat = vals.length > 0 && min === max;
            return {
              display: false,
              beginAtZero: !flat,
              min: flat ? Math.max(0, min * 0.85) : undefined,
              max: flat ? max * 1.15 : undefined,
            };
          })() },
        animation: false,
        events: []
...
function renderDonutRef(d) {
  ...
  const _pctsRef = pctMultiplo(_refItens.map(([,v]) => v), total);
  setHTML('dash-leg-ref', _refItens.map(([n,v,cor],i) =>
    `<div class="leg-item"><div class="leg-cor" style="background:${cor}"></div>
     <span class="leg-txt">${n}</span><span class="leg-val">${v}</span>
     <span class="leg-pct">(${total ? _pctsRef[i]+'%' : '—'})</span></div>`
  ).join(''));
}
...
function renderDistrib(d) {
  ...
  const _pctsDist = pctMultiplo(_distItens.map(([,v]) => v), total);
  setHTML('dash-leg-distrib', _distItens.map(([n,v,cor],i) =>
    `<div class="leg-item"><div class="leg-cor" style="background:${cor}"></div>
     <span class="leg-txt">${n}</span><span class="leg-val">${v}</span>
     <span class="leg-pct">(${total ? _pctsDist[i]+'%' : '—'})</span></div>`
  ).join(''));
}
...
function renderPeriodo(d) {
  ...
  // pctMultiplo garante soma exata = 100%
  const vals = itens.map(p => per[p.chave]);
  const pcts = pctMultiplo(vals, total);

  setHTML('dash-per-lista', itens.map((p, i) => {
    const v = vals[i], w = pcts[i];
