...
function classeRef(p) {
  if (!p) return '';
  const m = { cafe:'cafe', almoco:'almoco', jantar:'jantar',
    'café da manhã':'cafe','almoço':'almoco' };
  return m[p.toLowerCase?.() || p] || m[p] || '';
}

// ── Nome legível da refeição ─────────────────────────────
function nomePeriodo(p) {
  if (!p) return '—';
  const m = { cafe:'Café da Manhã', almoco:'Almoço', jantar:'Jantar',
    'café da manhã':'Café da Manhã', 'almoço':'Almoço', 'jantar':'Jantar' };
  return m[p.toLowerCase?.() || p] || m[p] || p;
}
...
// ── Arredondamento de múltiplos percentuais sem distorção ─
// Garante que a soma sempre resulte em 100%
// (algoritmo "largest remainder method")
function pctMultiplo(valores, total) {
  if (!total) return valores.map(() => 0);
  const exatos = valores.map(v => (v / total) * 100);
  const floors = exatos.map(Math.floor);
  const resto  = exatos.map((v, i) => v - floors[i]);
  let diferenca = 100 - floors.reduce((a, b) => a + b, 0);
  // Distribuir o resto para os maiores restos até fechar 100
  const idx = resto
    .map((r, i) => ({ r, i }))
    .sort((a, b) => b.r - a.r)
    .map(x => x.i);
  idx.slice(0, diferenca).forEach(i => floors[i]++);
  return floors;
}
...
function calcPctPos(dados) {
  ...
  // Retorna inteiro arredondado para exibição
  return totalAv > 0 ? Math.round(pos / totalAv * 100) : null;
}
...
function calcPctNeg(dados) {
  ...
  return totalAv > 0 ? Math.round(neg / totalAv * 100) : null;
}
